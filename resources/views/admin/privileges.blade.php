...
@section('content')
                    ...
                    <form role="form" method="post" action="{{$url}}/search" onsubmit="return r_submit(this)">
                        <div class="col-md-12">
                            <div class="form-group col-md-4">
                                <input type="text" name="code" value="{{$code}}" class="form-control" id="exampleInputEmail1" placeholder="权限元编号" />
                            ...
                            <div class="form-group col-md-4">
                                <select name="typecode" class="form-control">
                                    ...
                                    @foreach($typelist as $k=>$v)
                                        <option value="{{$k}}" @if($k==$typecode) selected @endif>{{$v}}</option>
                                    @endforeach
                                </select>
                                </div>
                            <div class="form-group col-md-4">
    ...
    <div class="row">
        ...
        <div class="box col-md-12">
            <div class="box-inner">
                <div class="box-header well" data-original-title="">
                    <h2><i class="glyphicon glyphicon-user"></i> 权限元管理</h2>
                    ...
                    <form action="" name="form1" method="post"  onsubmit="return r_submit(this)">
                        <table class="table table-striped table-bordered">
                            ...
                            <tr>
                                ...
                                <td>{{$r->name}}</td>
                                <td class="center">
                                    @if(isset($typelist[$r->type_code]))
                                        {{$typelist[$r->type_code]}}
                                    @else
                                        未知分类
                                    @endif
                                </td>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group col-md-12">
                            <label for="form_type_code">分类</label>
                            <select name="type_code" id="form_type_code" class="form-control">
                                @foreach($typelist as $k=>$v)
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $("#op_add_form").click(function(e){
                e.preventDefault();
                $(".typecode").html("<input type=\"text\" class=\"form-control\" id=\"form_code\" name=\"code\" placeholder=\"权限元编号\">");
                $("#form_name").val("");
                $("#form_type_code").val("{{$typecode}}");
            ...
            $(".data_mod").click(function(e){
                ...
                $.get(url, function(result){
                    ...
                    $(".typecode").html("<label>编号&nbsp;</label>"+myobj.code);
                    $("#form_name").val(myobj.name);
                    $("#form_type_code").val(myobj.type_code);
